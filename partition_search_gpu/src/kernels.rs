...
unsafe fn partition_search(
    ...
    while *next_partition_idx < usize::MAX {
        ...
        for p_idx in 0..PARTITIONS_COUNT {
            ...
            if *(partition_votes.add(p_idx)) < usize::MAX {
                ...
                if dist2 < nn_dist2 {
                    ...
                    // conditions and inaccurate vote counts, but this is ok since the vote counts
        ...
        if grid_thread_idx == 0 {
            ...
            // Record the vote winner, or use usize::MAX to indicate that no partition
            // received any votes.
        ...
        if *next_partition_idx < usize::MAX {
            ...
            let sorted_data_to =
                ((*next_partition_idx + 1) * partition_size).min(sorted_object_indices.len());
            ...
            for sorted_data_idx in sorted_data_from..sorted_data_to {
                ...
                // If we're trying to examine the first point in the cache, then load the cache
                // first.
                if cache_idx == 0 {
                    thread::sync_threads();
